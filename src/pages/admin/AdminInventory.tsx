...
const AdminInventory: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p style={{ color: COLORS.muted }}>
            {inventory.length} items · {alertCount} alertas
          </p>
        </div>
        <Button
          ...
          <p className="text-sm text-red-300">
            {totals.out > 0 && (
              <span className="font-bold">
                {totals.out} item{totals.out !== 1 ? "s" : ""} agotado
                {totals.out !== 1 ? "s" : ""}.{" "}
              </span>
            )}
            {totals.low > 0 && (
              <span>
                {totals.low} item{totals.low !== 1 ? "s" : ""} con stock bajo.
              </span>
            )}{" "}
      ...
      {/* Filters */}
      <Card
        className="p-4 flex gap-4 flex-wrap items-center"
        style={{
      ...
      >
        ...
        <div className="flex gap-2 flex-wrap w-full sm:w-auto overflow-x-auto pb-1">
          {(
            [
            ...
            ] as { value: typeof filterStatus; label: string; count: number }[]
          ).map((opt) => (
            <button
              key={opt.value}
            ...
            <tbody>
              {filtered.map((item) => (
                <motion.tr
                  layout
                  key={item.id}
                  className="border-b hover:bg-white/2"
              ...
              {[
                ...
                <div key={field.key} className="space-y-1">
                  ...
                  <input
                    type={field.type}
                    value={
                      (editItem as Record<string, unknown>)[field.key] as
                        | string
                        | number
                    }
                    onChange={(e) => {
