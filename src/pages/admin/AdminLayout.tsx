...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
            ...
            <div className="p-6 border-b" style={{ borderColor: COLORS.border }}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs" style={{ color: COLORS.muted }}>{user?.name}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 p-4 space-y-1 overflow-y-auto">
                {visibleNavItems.map(item => (
                    <Link
                        ...
                        style={isActive(item.path, item.exact)
                    ...
                    >
                        {item.icon}
                        {item.label}
                        {isActive(item.path, item.exact) && <ChevronRight size={14} className="ml-auto" />}
                    </Link>
                ))}
            </div>
    ...
    return (
        <div className="min-h-screen flex" style={{ backgroundColor: COLORS.bg }}>
            {/* Desktop Sidebar */}
            <aside className="hidden lg:block w-64 shrink-0 border-r" style={{ backgroundColor: COLORS.secondary, borderColor: COLORS.border }}>
