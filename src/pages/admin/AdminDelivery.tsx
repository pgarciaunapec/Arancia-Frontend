...
const AdminDelivery: React.FC = () => {
  ...
  const loadDeliveries = async () => {
    ...
    setError('');
    try {
      const response = await apiRequest<ApiEnvelope<DeliveryRecord[]>>('/admin/delivery', { auth: true });
      setDeliveries(response.data || []);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'No se pudo cargar delivery');
      setDeliveries([]);
    } finally {
      setLoading(false);
    }
  ...
  const activeCount = useMemo(() => deliveries.filter((item) => ['pending', 'assigned', 'in_transit'].includes(item.status)).length, [deliveries]);
            ...
            <table className="w-full min-w-[680px] text-sm">
              ...
              <tbody>
                {deliveries.map((delivery) => {
                  ...
                  return (
                    <tr key={delivery._id} className="border-b border-white/5 text-white/90">
                      <td className="p-3 font-mono text-xs">#{delivery.order?._id?.slice(-6) || delivery._id.slice(-6)}</td>
                      <td className="p-3">{delivery.user?.name || 'Cliente'}</td>
                      <td className="p-3">{STATUS_LABEL[delivery.status]}</td>
                      <td className="p-3">{delivery.estimatedMinutes ? `${delivery.estimatedMinutes} min` : '-'}</td>
                      <td className="p-3">
                        {nextStatus ? (
                          <Button size="sm" variant="outline" className="border-white/20 text-white" onClick={() => updateStatus(delivery, nextStatus)}>
