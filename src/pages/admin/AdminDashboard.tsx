            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="p-6" style={{ backgroundColor: COLORS.secondary, border: `1px solid ${COLORS.border}` }}>
                    <h3 className="font-bold text-white mb-4 flex items-center gap-2">
                        <ShoppingBag size={16} style={{ color: COLORS.primary }} /> Últimos Pedidos
                    </h3>
                    <div className="space-y-3">
                        {recentOrders.map(order => (
                            <div key={order.id} className="flex items-center justify-between text-sm py-2 border-b border-white/5">
                                <div>
                                    <p className="text-white font-medium">#{order.id.split('-').slice(-2).join('-')}</p>
                                ...
                                <div className="text-right">
                                    <p style={{ color: COLORS.primary }} className="font-bold">RD${order.total.toFixed(0)}</p>
                        ...
                        {recentOrders.length === 0 && (
                            <p className="text-center py-6 text-sm" style={{ color: COLORS.muted }}>Sin pedidos aún</p>
                        )}
                    </div>
                </Card>
                ...
                <div className="space-y-6">
                    <Card className="p-6" style={{ backgroundColor: COLORS.secondary, border: `1px solid ${COLORS.border}` }}>
                        <h3 className="font-bold text-white mb-4 flex items-center gap-2">
                            <Package size={16} style={{ color: COLORS.primary }} /> Alertas de Inventario
                        </h3>
                        {lowStock.length === 0 ? (
                            <p className="text-sm text-green-400">✓ Todo el inventario está al nivel correcto</p>
                        ) : (
                                ...
                                {lowStock.slice(0, 5).map(item => (
                                    <div key={item.id} className="flex items-center justify-between text-sm">
                                        ...
                                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                            item.status === 'out' ? 'text-red-400 bg-red-500/10' : 'text-yellow-400 bg-yellow-500/10'
                                        }`}>
                                            {item.status === 'out' ? 'Agotado' : `Bajo (${item.quantity} ${item.unit})`}
                                        </span>
                                    </div>
                                ))}
                            </div>
                        ...
                        {cashSession ? (
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className={cashSession.isOpen ? 'text-green-400' : 'text-gray-400'}>
                                        {cashSession.isOpen ? '● Abierta' : '○ Cerrada'}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span style={{ color: COLORS.muted }}>Balance inicial</span>
                                    <span className="text-white">RD${cashSession.openingBalance.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span style={{ color: COLORS.muted }}>Ventas totales</span>
                                    <span style={{ color: COLORS.primary }}>RD${cashSession.totalSales.toLocaleString()}</span>
                                </div>
                            </div>
                        ) : (
                            <p className="text-sm" style={{ color: COLORS.muted }}>No hay sesión de caja abierta</p>
                        )}
                    </Card>
                </div>
            </div>
