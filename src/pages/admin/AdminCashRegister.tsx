...
const AdminCashRegister: React.FC = () => {
  ...
  const todayCard = todayOrders
    .filter((o) => o.transaction?.method === "card")
    .reduce((s, o) => s + o.total, 0);
  const todayCash = todayOrders
    .filter((o) => o.transaction?.method === "cash")
    .reduce((s, o) => s + o.total, 0);
  const todayTransfer = todayOrders
    .filter((o) => o.transaction?.method === "transfer")
    .reduce((s, o) => s + o.total, 0);
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          ...
          <div
            className={`px-3 py-1 rounded-full text-xs font-bold ${cashSession?.isOpen ? "text-green-400 bg-green-500/10" : "text-red-400 bg-red-500/10"}`}
          >
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-end">
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (!Number.isFinite(amount) || amount < 0) {
                  window.alert("Por favor ingresa un monto de cierre válido.");
                  return;
                }
                closeCashSession({
                  notes: "Cierre desde panel administrativo",
                  declaredClosingBalance: amount,
                });
                setCloseAmount("");
              }}
              ...
              {todayOrders
                ...
                .map((order) => {
                  ...
                  return (
                    <tr
                      key={order.id}
                      className="border-b hover:bg-white/3"
