...
const Events: React.FC<EventsPageProps> = ({ onShowModal }) => {
  const requestQuote = async (eventType: string, packageName?: string) => {
    ...
    if (!phone) return;
    const guestsRaw = window.prompt('Cantidad estimada de invitados:', '30') || '30';
    ...
    try {
      ...
      onShowModal('¡Solicitud Recibida!', 'Tu solicitud fue enviada correctamente. Te contactaremos pronto.');
    } catch (error) {
      onShowModal('Error', error instanceof Error ? error.message : 'No se pudo enviar la solicitud.');
    }
  };
