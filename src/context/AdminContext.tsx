...
const mapCashRegister = (raw: any): CashSession => ({
  ...
  closingBalance:
    raw.closingBalance !== undefined ? Number(raw.closingBalance) : undefined,
...
export const AdminProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const refreshAlerts = useCallback(async () => {
    ...
    const alertsResponse = await apiRequest<ApiEnvelope<any[]>>(
  ...
  const refreshAdminData = useCallback(async () => {
    ...
    const [tablesResponse, cashResponse, inventoryResponse, alertsResponse] =
      await Promise.all([
        apiRequest<ApiEnvelope<any[]>>("/admin/tables", { auth: true }),
        apiRequest<ApiEnvelope<any | null>>("/admin/cash-register/today", {
          auth: true,
        }),
        canManageInventory
        ...
        canManageInventory
          ? apiRequest<ApiEnvelope<any[]>>("/admin/inventory/alerts", {
              auth: true,
            })
          : Promise.resolve({ data: [] } as ApiEnvelope<any[]>),
      ]);
    ...
    setLowStockAlerts((alertsResponse.data || []).map(mapBackendInventoryItem));
    setCashSession(
      cashResponse.data ? mapCashRegister(cashResponse.data) : null,
    );
  ...
  const addInventoryItem = useCallback(
    async (item: Omit<InventoryItem, "id" | "lastUpdated" | "status">) => {
  ...
  const closeCashSession = useCallback(
    async (payload?: { notes?: string; declaredClosingBalance?: number }) => {
