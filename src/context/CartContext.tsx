...
const normalizeStoredCartItem = (
  ...
  return {
    ...
    image:
      ...
      "",
    category:
      (typeof raw.category === "string" && raw.category.trim()) || "General",
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const hydrateFromBackend = useCallback(async () => {
    const response = await apiRequest<ApiEnvelope<any>>("/cart", {
      auth: true,
    });
    ...
    (menuItem: MenuItem) => {
      if (isAuthenticated) {
        apiRequest<ApiEnvelope<any>>("/cart", {
          ...
          auth: true,
          body: JSON.stringify({
            menuItemId: menuItem.backendId || menuItem.id,
            quantity: 1,
          }),
        })
          .then((response) => {
