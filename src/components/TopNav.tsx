                ...
                <div className="flex items-center justify-between px-2 py-1.5">
                    ...
                    <div className="flex items-center gap-1 mr-1">
                        ...
                        <Link to="/my-reservations" className="p-2 rounded-full hover:bg-white/10 transition-colors group" title="Mis Reservas">
                            <CalendarCheck size={18} className="text-white group-hover:text-[#f5b400] transition-colors" />
                        </Link>

                        {/* My Orders */}
                        {isAuthenticated && (
                            <Link to="/my-orders" className="p-2 rounded-full hover:bg-white/10 transition-colors group" title="Mis Pedidos">
                                <Package size={18} className="text-white group-hover:text-[#f5b400] transition-colors" />
                            </Link>
                        )}

                        {/* Cart with count */}
                        <Link to="/cart" className="p-2 rounded-full hover:bg-white/10 transition-colors relative group" title="Carrito">
                            <ShoppingBag size={18} className="text-white group-hover:text-[#f5b400] transition-colors" />
                            {count > 0 && (
                        ...
                        <div className="relative">
                            {isAuthenticated ? (
                                ...
                                <Link to="/login" className="p-2 rounded-full hover:bg-white/10 transition-colors group" title="Iniciar Sesión">
                                    <User size={18} className="text-white group-hover:text-[#f5b400] transition-colors" />
                                </Link>
                            )}
                            ...
                            {showUserMenu && isAuthenticated && (
                                <div
                                    ...
                                    style={{ backgroundColor: COLORS.secondary, borderColor: 'rgba(245,180,0,0.3)' }}
                                >
                                    ...
                                    {[
                                        ...
                                        { to: '/my-orders', icon: <Package size={14} />, label: 'Mis Pedidos' },
                                        { to: '/my-reservations', icon: <CalendarCheck size={14} />, label: 'Mis Reservas' },
                                        ...(isAdmin ? [{ to: '/admin', icon: <ShieldCheck size={14} />, label: 'Panel Admin' }] : []),
                                    ].map(item => (
